...
- **Input Strategy:** Deterministically generated elements containing negatives and duplicates. The generator uses an exact 32-bit linear congruential recurrence `seed = (Math.imul(seed, 1103515245) + 12345) | 0` starting at `12345`. To guarantee duplicates and mixed data without pathological distributions, the value is explicitly constrained to the exact range `[-5000, 5000]` using `seed % 5001`. In addition, to guarantee duplicate elements across all test dimensions, a deterministic duplicate rule copies the previously generated value whenever the condition `i > 0 && i % 7 === 0` is met.
