...
5. **Selection Policy & Runtime Selector (Phase 3):** 
   The core decision logic. Consumes characteristics from the Analyzer and a *frozen*, immutable `FrozenSelectionPolicy` to route the workload deterministically. It has no side effects, executes no benchmarks, and does not remember past behavior. Browser routing guarantees that exactly one executor is invoked.
